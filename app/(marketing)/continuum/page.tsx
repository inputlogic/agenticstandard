...
const includes = [
  {
    ...
    p1: "Phase 1 candidates from your Map ship on the cadence the roadmap calls for. That's typically one to two per quarter, more in heavy phases, fewer when the work is plumbing. Each deployment includes the same workflow shadowing, tuning, and stabilization rigor as your Foundation agents.",
    p2: "The Map itself stays alive. New candidates surface; phasing shifts as reality unfolds; the roadmap you're working from at month nine is sharper than the one delivered at week six.",
  },
  {
    num: '02',
    tag: 'The Fleet',
    title: <>Agent <em>stewardship</em>.</>,
    p1: "Every agent ever deployed for you is monitored, tuned, and evolved: the originals from the Foundation and every one that lands afterward. Production-grade Harbour hosting is included, with the failure handling, audit trails, and observability that a serious operator expects.",
    p2: "When something breaks, the fix lands without a bug report on your end. The fleet is the firm's responsibility, not yours.",
  },
  {
    num: '03',
    tag: 'The Counsel',
    title: <>Strategic <em>advisory</em>.</>,
    p1: "A named senior practitioner is your partner for the entire relationship. Regular working sessions with your sponsor and operational owner. A direct Slack channel (or equivalent) for the in-between moments when a decision can't wait for the next meeting.",
    p2: "This is the relationship that lets your sponsor make AI-related decisions with conviction, because there's someone they trust who has seen the room before.",
  },
  {
    num: '04',
    tag: 'The People',
    title: <>Workforce <em>coaching</em>.</>,
    p1: "The habitual work doesn't end at week six. Role-specific enablement continues: prompt library development, internal training-of-trainers, prompting evaluation, and coaching for the champions who own AI inside each department.",
    p2: "The end state is a workforce that doesn't need the firm anymore; not because the engagement ended, but because the organization has internalized what it means to work with AI as a default rather than an exception.",
  },
  {
    num: '05',
    tag: 'The Fences',
    title: <>Governance <em>maturation</em>.</>,
    p1: "Active work moving you up the governance dimension. Skills like risk-tiered policy authoring, audit infrastructure, scoped-credential design, incident response playbooks, and the periodic agent reviews that make agentic AI defensible to your board, your legal team, and your auditors.",
    p2: "This is where most consulting engagements stop short. Continuum leans into it, because bounded autonomy is the only autonomy that scales.",
  },
  {
    num: '06',
    tag: 'The Reporting',
    title: <>Quarterly <em>board-ready</em> reporting.</>,
    p1: "Your Map scorecard, updated and narrated, with a one-page executive summary your sponsor can place on a board agenda without rewriting. Hours saved, capacity unlocked, agents shipped, dimensions advanced, incidents handled.",
    p2: "Designed to be shown to your board without you having to assemble it yourself. The quarterly report is the artifact that proves the retainer is paying for itself, every quarter, in language your board already speaks.",
  },
  {
    num: '07',
    tag: 'The Plan',
    title: <>Roadmap <em>evolution</em>.</>,
    p1: "The Map is a living document. Every quarter the Map is refreshed: candidates that have shipped move to a track record; new candidates surface from the work; phases may shift as reality (and your priorities) change. Your roadmap at the end of year one is built on a year of grounded observation, not a single week of interviews.",
    p2: "If you ever choose to leave, the roadmap you leave with is sharper than anything a strategy firm could produce, because it's been refined by the work itself.",
  },
]

const cadence = [
  { freq: 'Weekly', title: 'Tactical channel', body: "Slack or equivalent. Agent monitoring, day-to-day questions, decisions that can't wait. Open async." },
  { freq: 'Bi-weekly', title: 'Working session', body: '60\u201390 minutes with the operational owner. Roadmap progress, current candidates, blockers to clear.' },
  { freq: 'Monthly', title: 'Sponsor review', body: "Executive-level alignment with the sponsor. What shipped, what's next, what they need from the firm." },
  { freq: 'Quarterly', title: 'Board-ready report', body: 'Refreshed Map, scorecard, and one-page executive summary. The artifact the sponsor takes to the board.' },
]
    ...
    <>
      ...
      <section className={s.hero}>
        <HeroDrift>
          ...
          <h1 className={`${s['hero-title']} rise`} style={{ animationDelay: '0.18s' }}>
            The Agentic <em>Continuum</em>.
          </h1>
          <div className={`${s['hero-subtitle']} rise`} style={{ animationDelay: '0.36s' }}>
      ...
      <section className={s.section}>
        ...
        <ScrollReveal delay={160} className={s.reading}>
          <p>
      ...
      {/* 7 Include articles */}
      {includes.map((item, i) => (
        <ScrollReveal as="article" key={item.num} delay={i * 80} className={`${s.include} ${i === includes.length - 1 ? s['include-last'] : ''}`}>
          <div className={s['include-grid']}>
            <div className={s['include-meta']}>
              <Parallax speed={i % 2 === 0 ? -0.09 : -0.05} maxShift={44}>
      ...
      <section className={s.section}>
        ...
        <div className={s.cadence}>
          {cadence.map((c, i) => (
            <ScrollReveal key={c.freq} delay={i * 80} className={s['cadence-cell']}>
      ...
      <section className={s.section}>
        ...
        <div className={s['scope-list']}>
          {outOfScope.map((text, i) => (
            <ScrollReveal key={i} as="div" delay={i * 60} className={s['scope-item']}>
      ...
      <div className={s['pricing-band']}>
        <ScrollReveal className={s['pricing-label-wrap']}>
          <SectionLabel accent>From</SectionLabel>
        </ScrollReveal>
        <ScrollReveal delay={90}>
          <Parallax speed={-0.06} maxShift={32}>
            <div className={s.price}>
              <CountUp to={20000} prefix="$" duration={1100} />
            </div>
          </Parallax>
          <div className={s['price-unit-block']}>Per month</div>
        </ScrollReveal>
